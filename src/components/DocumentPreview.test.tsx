...
describe("DocumentPreview", () => {
  it("renders an empty state if no images", () => {
    const { container } = render(
      <DocumentPreview
        images={[]}
        fallbackNode={<div data-testid="fallback">Fallback</div>}
      />,
    );
  ...
  it("renders multiple img tags if images array has items", () => {
    ...
    const { container } = render(<DocumentPreview images={images} />);
    expect(container.querySelectorAll("img")).toHaveLength(2);
    expect(
      screen.getByText('printer.previewPage{"page":1}'),
    ).toBeInTheDocument();
    expect(
      screen.getByText('printer.previewPage{"page":2}'),
    ).toBeInTheDocument();
  });
  ...
  it("renders N-up grid layout for 4-up", () => {
    ...
    expect(
      screen.getByText('printer.nupSheet{"sheet":1,"start":1,"end":4}'),
    ).toBeInTheDocument();
  });
  ...
  it("does not render N-up grid when nup is 1", () => {
    const images = ["p1.png", "p2.png"];
    const { container } = render(<DocumentPreview images={images} nup={1} />);
    expect(container.querySelectorAll("img")).toHaveLength(2);
