...
export default function JobsModal({ onClose }: JobsModalProps) {
  ...
  const fetchJobs = useCallback(async (silent = false) => {
    ...
    try {
      ...
      newData.sort(
        (a, b) =>
          parseGMTDate(b.submitted_at).getTime() -
          parseGMTDate(a.submitted_at).getTime(),
      );
      setJobs(newData);
      setError(null);
    } catch (err: unknown) {
  ...
  const getStatusBadge = (status: string) => {
    ...
    return (
      <span
        className={`px-2.5 py-1 text-xs font-semibold rounded-full ${color}`}
      >
        {t(`status.${status}`)}
      </span>
    );
          ...
          {loading && jobs.length === 0 ? (
            ...
            <div className="space-y-3">
              {jobs.map((job) => {
                ...
                const isPendingDuplex =
                  job.status === "pending_manual_continue";

                let jobTimeLeft = null;
                if (isPendingDuplex && job.hook_expires_at) {
                  jobTimeLeft = Math.max(
                    0,
                    parseGMTDate(job.hook_expires_at).getTime() - now,
                  );
                }
                ...
                return (
                  ...
                  >
                    <div className="flex items-start space-x-3 flex-1 min-w-0">
                      <div
                        className={`p-2 rounded-xl flex-shrink-0 mt-0.5 ${isPendingDuplex ? "bg-purple-50 text-purple-600" : isActive ? "bg-blue-50 text-blue-600" : "bg-gray-50 text-gray-500"}`}
                      >
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        <div className="flex flex-wrap items-center text-xs text-gray-500 mt-1 gap-x-2 gap-y-0.5">
                          ...
                          {job.submitted_at && (
                            <>
                              ...
                              <span>
                                {parseGMTDate(job.submitted_at).toLocaleString(
                        ...
                        {isPendingDuplex && jobTimeLeft !== null && (
                          <div className="mt-2 text-xs font-medium text-orange-600 bg-orange-50 px-2 py-1 inline-flex items-center rounded-md">
                            ...
                            {t("printer.timeLeft")}{" "}
                            {Math.max(0, Math.floor(jobTimeLeft / 1000 / 60))}:
                            {Math.max(0, Math.floor((jobTimeLeft / 1000) % 60))
                              .toString()
                              .padStart(2, "0")}
                          </div>
